...
namespace Ladybug_Mayhem
{
    public class DrawBG
    {
...

        {
            ...
            roofNorthWest = new DrawObject(game, content, "Roof North West", 1, new Vector2(0, - (171 / 2)), 1);
            roofWest = new DrawObject(game, content, "Roof West", 1, new Vector2(0, 0), 1);
            roofSouthWest = new DrawObject(game, content, "Roof South West", 1, new Vector2(0, roofWest.GetPosition().Y + (roofWest.GetSprite().Bounds.Height / 2)), 1);
            wallBlockTall = new DrawObject(game, content, "Wall Block Tall", 7, new Vector2(0, roofSouthWest.GetPosition().Y + (roofSouthWest.GetSprite().Bounds.Height / 2)), 1);
            roofNorth = new DrawObject(game, content, "Roof North", 5, new Vector2(roofNorthWest.GetPosition().X + roofNorthWest.GetSprite().Bounds.Width, - (171 / 2)), 0);
            ...
            roofNorthEast = new DrawObject(game, content, "Roof North East", 1, new Vector2
                (roofNorth.GetPosition().X + (roofNorth.width * 5), 0 - (roofNorthEastTexture.Bounds.Height / 2)), 0);
            brownBlock = new DrawObject(game, content, "Brown Block", 4, new Vector2(roofWest.GetPosition().X + roofWest.GetSprite().Bounds.Width, 
                roofNorth.GetPosition().Y + (roofNorth.GetSprite().Bounds.Height) / 2), 1);
            roofSouth = new DrawObject(game, content, "Roof South", 5, new Vector2(roofSouthWest.GetPosition().X + 
                roofSouthWest.GetSprite().Bounds.Width, brownBlock.GetPosition().Y + (roofNorth.GetSprite().Bounds.Height) / 2), 0);
            roofEast = new DrawObject(game, content, "Roof East", 1, new Vector2(windowWidth - (roofNorthEast.GetSprite().Bounds.Width * 2), 0), 1);
            roofSouthEast = new DrawObject(game, content, "Roof South East", 1, new Vector2(windowWidth - (roofEast.GetSprite().Bounds.Width * 2), 0), 1);
